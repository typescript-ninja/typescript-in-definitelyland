...
@<code>{--noEmitOnError}オプションについて解説します。
このオプションはコンパイルに成功した時のみ成果物である.jsファイル、.js.mapファイル、.d.tsファイルを生成します。

gruntやgulpなどのタスクランナーを利用する際の「コンパイル成功したつもりだったけど失敗してた。後続のタスクが続いてしまい失敗を見逃した。」というパターンに有効です。
...
@<code>{--noEmit}オプションについて解説します。
@<code>{--noEmitOnError}オプションと違い、コンパイルが成功しようが失敗しようが、常に成果物を何も生成しません。

#@# prh:disable
@<code>{--noEmit}オプションは使いどころが限られます。
webpackでts-loaderを使っている場合など、@<code>{tsc -p ./}でファイルを生成するのとは違う手順でのみビルドを行うプロジェクトで有効です。
@<code>{tsc -p ./ --noEmit}とすることでビルドタスク全体を走らせることなく、TypeScriptのコンパイルエラーのみをチェックする目的で利用できます。
これにより、作業ディレクトリに余計なファイルを撒き散らすことなくコンパイルが通るかどうか試せます。
