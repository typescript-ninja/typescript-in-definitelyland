...
=== --strictNullChecks 有効・無効時の振る舞い

union typesについて解説したので、@<code>{--strictNullChecks}オプションについても触れておきます。
本書では、@<code>{--strictNullChecks}オプションは常に有効にしている前提で書いています。
もし、このオプションを使わないときはTypeScriptコンパイラがどういう解釈を行うかを確認します。

まずは、本書標準のオプションありのときの例です（@<list>{unionTypes/strictNullChecks}）。

//list[unionTypes/strictNullChecks][危険なコードがいち早く発見される]{
#@mapfile(../code/types-advanced/unionTypes/strictNullChecks.ts)
...
さて、次はオプションなしのときの例です（@<list>{unionTypes/withoutStrictNullCheck-ignore}）。

//list[unionTypes/withoutStrictNullCheck-ignore][実行時にエラーになるかも]{
#@mapfile(../code/types-advanced/unionTypes/withoutStrictNullCheck-ignore.ts)
