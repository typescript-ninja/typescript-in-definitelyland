...
しかし、筆者はMax OS XユーザなのでVisual Studioが選択肢から外れてしまいます。
...
つまりどういうことかというと、ある1つのファイルの中に、全ての依存関係を解決するreference commentを書いてまわらないといけないのです。
筆者も最初はこの方針で作業していたのですが、管理がものすごくめんどくさいこと、循環依存が発生した時に--outオプションを併用した時の出力順制御が難しいこと、TypeScript 1.1.0-1からコンパイル時間が大幅に改善したのでコンパイル対象が大きくても気にならなくなったこと、上記3点を踏まえて、rootになるファイル1つ定め、それを頂点にtree上に依存関係を定義するようにしました。
File Watcherやgruntなどのツールでは、このrootとして設計したファイルのみを常にコンパイルするようにします。

 * 必要があれば --out を設定

依存関係をtree状に綺麗に定義した場合として、ファイルの出力結果が予想しやすくなります。
