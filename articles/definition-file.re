...
#@# REVIEW muo: 少々言葉不足でつながり悪く感じた。「拡張子を.d.tsとしたファイルに実装を含むようなコードを書くと」かなぁ
...
#@# REVIEW muo: 後付で手書きで とつながるのが嫌な感じ。「JavaScriptに後付の型定義を手書きしていくため」かな。気に食わなかったらスルーでok
...
#@# REVIEW muo: power-assetどっから出てきたの!? となるので、「ここでは〜〜なpower-assertを題材として型定義ファイルの〜〜をやってみましょう」的な前説を付近に追加すべき。
...
#@# REVIEW muo: こう書くならtsdについても一言紹介しておいたほうが良いように思った
...
#@# REVIEW muo: コマンドでソースをコンパイル あたりが良さそう(言葉足りてない)
リファレンスコメントとして参照するほうがコンパイル手順を簡素に保てるため適しています。
#@# REVIEW muo: 開発に適す?運用に適す?どういうことか補記あったほうが嬉しい
...
#@# REVIEW muo: 社内製 内製 社内 は良いけど社内内製はいけない
...
#@# REVIEW muo: JavaScriptで書いて→JavaScriptでコードを書いて
#@# REVIEW muo: 「当てて使う」の表現は少々感覚寄りすぎかなーと
...
#@# REVIEW muo: 「型を使って間違ったコードを書きにくいように」の部分が構文解析しづらいので、ちょい冗長になるけど「型指定をおこなうことで間違った」のほうが良さそう
#@# REVIEW muo: 納得できると思います→納得できるでしょう(または、納得がいくでしょう)

人は誰しも"上手く書けないので仕方なく"悪い型定義を書いてしまうことがあります。
#@# REVIEW muo: 人は誰しも→人間、どうしても あたり
...
#@# REVIEW muo: 「最終的に、良し悪しが分かるようになるには」→「型定義ファイルの良し悪しを判断する力は」
...
#@# REVIEW muo: 実体のある→実体を持つ
#@# REVIEW muo: 相乗りしていってしまったほうが→相乗りしてしまったほうが
...
#@# REVIEW muo: うかつにJQueryStaticからjquery.Staticに変更すると(なにを?)
...
#@# REVIEW muo: 使える人は→使えるのは
少なくとも、この型定義ファイルをヒントに実際のコードを書くことは大いなる苦痛が伴います。
#@# REVIEW muo: ×実際のコードを書くことは大いなる苦痛が ○実際のコードを書くことには大いなる苦痛が ○実際のコードを書くには大いなる苦痛が ○実際のコードを書くことは大いなる苦痛を
俺は絶対使わんぞ！
...
#@# REVIEW muo: 層構造→階層構造 かな、ここの流れだと。
...
#@end
//}
#@# REVIEW muo: 前のとキャプション同じだけど意図してる? 話的にはつながってるので良いかもしらんけど念のため
...
#@# REVIEW muo: 上記2点のURL端っこ入る気がしないのでハッシュ削るなりしたほうが良い
...
#@# REVIEW muo: 危険度としては〜とするなら「同程度です。」で止める。逆に「危険です。」で止めるなら「危険度としては」を削る
素直にanyを使いましょう。
...
#@# REVIEW muo: ここ1個目の"jquery"は意図してる?それともjQuery?
コントリビュートの輪！
...
TypeScriptでコードを書く上では、1ファイル1インタフェースorクラスの図式でプロジェクトを構成することがありえます。
一方、インタフェース単体をexportするのって、既存JavaScriptファイルに対する型定義ファイルではありえない操作と言えます。
#@# REVIEW muo: ここの2行ゆるっゆるで文意がババロア感してるので一度見直しplz
...
#@# REVIEW muo: 匂い→臭いかな。ネガティブなニュアンスなので
...
== Let's contribution!
#@# REVIEW muo: Let'sに噛ませるなら contribute! では
